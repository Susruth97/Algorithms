...
void typeCastToAllDataTypes(void * long_ptr, void * float_ptr, void * char_ptr, void * str_ptr) {
	...
	//printf("Value stored at char_ptr is %c\n", *(char *)char_ptr);
	//printf("Value stored at char_ptr is ");
	char *c;
	c = (char *)str_ptr;
	//for (int i = 0; c[i] != '\0'; i++)
	//printf("%c", c[i]);
...
void task7() {
	...
	int *i;
	char *c;
	f = (float *)getFloatAsVoidStar();
	i = (int *)getIntAsVoidStar();
	c = (char *)getStringAsVoidStar();
	//printf("Float is %f\n", *(float *)f);
	//printf("Int is %d\n", *(int *)i);
	//printf("String is ");
	//for (int i = 0; c[i] != '\0'; i++)
...
void ** getTwoGreater(void *addr1, void *addr2, void *addr3, int(*compare)(void *, void *)) {
	...
	if ((*compare)((void *)addr1, (void *)addr2) == 1 && (*compare)((void *)addr1, (void *)addr3) == 1)
	{
		result[0] = addr1;
		if ((*compare)((void *)addr3, (void *)addr2) == 1)
			result[1] = addr3;
		else
			result[1] = addr2;
	}

	if ((*compare)((void *)addr2, (void *)addr1) == 1 && (*compare)((void *)addr2, (void *)addr3) == 1)
	{
		result[0] = addr2;
		if ((*compare)((void *)addr3, (void *)addr1) == 1)
			result[1] = addr3;
		else
			result[1] = addr1;
	}

	if ((*compare)((void *)addr3, (void *)addr2) == 1 && (*compare)((void *)addr3, (void *)addr1) == 1)
	{
		result[0] = addr3;
		if ((*compare)((void *)addr1, (void *)addr2) == 1)
			result[1] = addr1;
		else
			result[1] = addr2;
	}
