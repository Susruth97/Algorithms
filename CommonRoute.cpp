...
char * find_common_route(char * hacklist, char *codelist, int *cost){
	...
	*cost = 0;
	...
	{
		int len1, len2, index, scost, common = 0, mcost = -1, max = -1;

		len1 = find_len(hacklist);
		len2 = find_len(codelist);

		char *output;
		output = (char*)calloc(len1, sizeof(char));
		...
		for (int i = 0; i < len1; i++)
		{
			for (int j = 0; j < len2; j++)
			{
				...
				{
					if (hacklist[i + k] == codelist[j + k])
						scost = scost + hacklist[i + k] - 'A' + 1;
					else
						break;
				}
				if (k > max)
				...
				else if (k == max)
				{
					...
					{
						if (hacklist[i] - 'A' < hacklist[index] - 'A')
						{
							mcost = scost;
							max = k;
							index = i;
				...
				if (i + k == len1 && j + k == len2 && k == max)
					i = len1;
			}
		}

		int i = index, j = max, l = 0;
		while (i < j)
		{
			output[l] = hacklist[i];
			i++;
